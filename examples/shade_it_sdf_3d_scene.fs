...
float map(in vec3 pos) 
{
  ...
  float sphere = sdSphere(pos - sphere_pos, 0.25); 

  // box
  vec3 q = pos;
  //q = fract(pos) - 0.5; // repetition
  vec3 box_pos = vec3(-0.4, 0.0, 0.0);
  vec3 box_pos_transform = q - box_pos;
  box_pos_transform.xy *= rot2d(iTime);
  
  //box_pos_transform = fract(box_pos_transform) - 0.5;
  
  float box = sdBox(box_pos_transform, vec3(0.15));

  // pyramid
  vec3 py_pos = vec3(0.0, 0.5, -0.85);
  vec3 py_pos_transform = q - py_pos;
  	
  float scaling = 1.0;
  float pyramid = sdOctahedron(py_pos_transform / scaling, 0.25) * scaling;

  // ground
  float ground = pos.y - (-0.25); 

  return smin(ground,smin(smin(sphere, box, 0.4), pyramid, 0.9), 0.2);
}
